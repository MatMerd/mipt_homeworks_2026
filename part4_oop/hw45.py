...
class LFUPolicy(Policy[K]):
    ...
    def get_key_to_evict(self) -> K | None:
        if len(self._key_counter) > self.capacity:
            keys = list(self._key_counter)[: self.capacity]
            return min(keys, key=lambda k: self._key_counter[k])
        return None
...
class MIPTCache(Cache[K, V]):
    ...
    def set(self, key: K, value: V) -> None:
        ...
        key_to_remove = self.policy.get_key_to_evict()
        if key_to_remove is not None:
            self.storage.remove(key_to_remove)
            self.policy.remove_key(key_to_remove)

    def get(self, key: K) -> V | None:
        get_value = self.storage.get(key)
